...
function encontrarPalabraMasCorta(listaDePalabras) {
    return listaDePalabras.reduce((shortest, word) => (word.length < shortest.length ? word : shortest), listaDePalabras[0]);
}
...
function enumerarLista(listaDePalabras) {
    ...
    const lastWord = listaDePalabras.pop();
    if (listaDePalabras.length === 0) return lastWord + ".";
    return listaDePalabras.join(", ") + " y " + lastWord + ".";
}
